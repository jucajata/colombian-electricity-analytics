...
def etl_pod(date_from:dt.date=None, date_to:dt.date=None):
    ...
    df = objetoAPI.get_collections('PrecOferDesp') # El método get_collection con argumentos retorna los cruces de las varibles que se quieren consultar
    print(df.to_markdown())

    Id: 12, 
    MetricId: 'PrecOferDesp',
    MetricName: 'Precio de Oferta de Despacho por Recurso', 
    Entity: 'Recurso',
    MaxDays: 31, 
    Type: 'HourlyEntities',
    ...
    MetricUnits: 'COP/kWh',
    MetricDescription: 'Es el precio de la energia de un recurso de generación despachados centralmente enviados por los agentes para el Despacho. Incluye el CEE'
    '''
    ...
    for row in df.index:
        ...
        sql = '''INSERT INTO precio_oferta_despacho 
            (fecha,
                values_code, 
                values_hour01,
                values_hour02,
        ...
        try:  # identificamos si ya existe en la bd
            cur.execute(f"SELECT * FROM precio_oferta_despacho WHERE fecha='{fecha}' AND values_code='{values_code}';")
            r = len(cur.fetchall())
            if r == 0:
                cur.execute(sql, val)
                conn.commit()
